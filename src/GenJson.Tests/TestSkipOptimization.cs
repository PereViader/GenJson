...
public class TestSkipOptimization
{
    ...
    [Test]
    public void TestSerialization_WithCountProperty()
    {
        var obj = new SkipOptimizationClass([1, 2], [3, 4], new() { { "5", 6 } });
        var json = obj.ToJson(useCountOptimization: true);

        // Expect count properties
        var expected = """{"$List":2,"List":[1,2],"$Array":2,"Array":[3,4],"$Dictionary":1,"Dictionary":{"5":6}}""";
        Assert.That(json, Is.EqualTo(expected));
    ...
    [Test]
    public void TestDeserialization_WorksWithCountProperty()
    {
        var json = """{"$List":2,"List":[1,2],"$Array":2,"Array":[3,4],"$Dictionary":1,"Dictionary":{"5":6}}""";
        var obj = SkipOptimizationClass.FromJson(json, useCountOptimization: true)!;
        ...
        var utf8Json = System.Text.Encoding.UTF8.GetBytes(json);
        var utf8Obj = SkipOptimizationClass.FromJsonUtf8(utf8Json, useCountOptimization: true)!;
        Assert.That(utf8Obj, Is.Not.Null);
        Assert.That(utf8Obj.List, Is.EqualTo(new List<int> { 1, 2 }));
        Assert.That(utf8Obj.Array, Is.EqualTo(new int[] { 3, 4 }));
        Assert.That(utf8Obj.Dictionary, Is.EqualTo(new Dictionary<string, int> { { "5", 6 } }));
    ...
    [Test]
    public void TestDeserialization_WorksWithoutCountProperty_EvenIfOptimizationEnabled()
    {
        var json = """{"List":[1,2],"Array":[3,4],"Dictionary":{"5":6}}""";
        var obj = SkipOptimizationClass.FromJson(json, useCountOptimization: true)!;
